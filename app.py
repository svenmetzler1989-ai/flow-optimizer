...
# =====================================================================
# 2. STARTPARAMETRAR OCH VOLYMER (NYA NOLLSTÄLLDA STARTVÄRDEN)
# =====================================================================
START_INBOUND_STOCK = 0       # ⚡ ÄNDRAT TILL 0 (Kommer in under dagen)
START_INBOUND_NON = 0         # ⚡ ÄNDRAT TILL 0 (Kommer in under dagen)
START_PUTAWAY_STOCK = 0       # ⚡ ÄNDRAT TILL 0 (Börjar tomt)
START_PUTAWAY_NON = 0        # ⚡ ÄNDRAT TILL 0 (Börjar tomt)
START_PICK_STOCK = 10000      # 10 000 ordinarie orderrader att plocka
START_PICK_NON = 400          # ⚡ ÄNDRAT FRÅN 750 TILL 400 Non-Stock orderrader
START_PACK = 0                # ⚡ ÄNDRAT TILL 0 (Packkön börjar helt tom)
...
@st.cache_data
def fetch_live_data():
    # Tvingande logik som skriver över eventuella gamla värden från databasen
    return {
...
if live_sim and st.session_state.sim_minutes < 1410:
    ...
    st.session_state.db_data["putaway_non_stock"] = max(0, st.session_state.db_data["putaway_non_stock"] - inlagrat_non)
        ...
        # --- INKOMMANDE GODS UNDER DAGEN (FIXAT SYNTAXFEL) ---
    if m <= 1100 and random.random() > 0.93:
        st.session_state.db_data["inbound_stock"] += random.randint(1, 2)
    
    # Skottsäker logik för att hämta eller skapa variabeln i minnet
...
prognos_data = {
    ...
    "Tid till tomt (Timmar)": [
        ...
        "Automatisk", "Klar 14:30", "Löpande (Max 1.5h)",
